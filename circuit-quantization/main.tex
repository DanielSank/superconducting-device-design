...
\begin{align}
	...
	E^\mathrm{JJ}_\mathrm{nonlin} &= E_J (1 - \cos\phi - \frac{1}{2}\phi^2)	
\end{align}
We haven't done anything fancy here, just added and subtracted $E_J \phi^2/2$, but in doing so we have now defined a linear inductance plus a purely nonlinear element. (DOES THIS HAVE A NAME ALREADY?)

We can remove the nonlinear element and treat the rest of the circuit (including the linear inductance from the JJ) as a \textit{black box} with two ports that the nonlinear element will span. We can then calculate the impedance between these ports $Z(\omega)$. This calculation can be analytical or a simple numerical circuit simulation if the rest of the circuit elements have simple geometries, but in general may require a full-wave electromagnetic simulation of the physical device geometry. 

Once the impedance is known, the black box circuit can be replaced by an equivalent lumped-element circuit network that reproduces the same $Z(\omega)$ across those ports. 
