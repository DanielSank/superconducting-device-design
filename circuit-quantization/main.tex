...
Let's go back to a classical circuit, and for now let's pretend we don't have any loops in the circuit so that it's easy to write down the Lagrangian. We can use the Euler-Lagrange equations to find the equations of motion:
...
\begin{equation}
\mathcal{L}(\tilde{\vec{\Phi}}, \dot{\tilde{\vec{\Phi}}},t) = \frac{1}{2}\dot{\tilde{\vec{\Phi}}}^\top \mathbf{\tilde{C}}\dot{\tilde{\vec{\Phi}}} - \frac{1}{2}\tilde{\vec{\Phi}}\mathbf{L^{-1}}\tilde{\vec{\Phi}}
\end{equation}
